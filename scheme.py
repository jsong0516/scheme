...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError("Error: Bad arguments")
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len_formals != len(vals):
            raise SchemeError("Error: the number of formals is not equal to the number of vals")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if not scheme_symbolp(target[0]):
            raise SchemeError("Error: Bad name")
        fn = scheme_eval(Pair("lambda", Pair(target.second, vals.second)), env)
        env.define(target[0], fn)
        return target[0]
    else:
...
def do_let_form(vals, env):
    ...
    for ele in bindings:
        names, values = Pair(ele[0], names), Pair(scheme_eval(ele[1], env), values)
    check_formals(names)
...
def check_formals(formals):
    ...
    duplicate = []
    for ele in formals:
        if not scheme_symbolp(ele) or ele in duplicate:
            raise SchemeError("Invliad parameter")
